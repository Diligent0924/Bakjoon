...
나의 문제점
1. 문제를 정확하게 읽지 않고 지나가기 때문에 항상 고생한다.
2. 이번 문제는 그냥 90도를 도는 것이 아니라 전체가 90도 도는 방식이었다.
3. 문제를 제대로 알고 난 후에는 구현력이 나쁘지는 않으나 문제 자체의 읽는 방식을 더 길러야 한다.
4. 문제를 조금 더 고찰한 후에 나타내야 할 것 같다.
